...
见图\ref{KMBHlattice}。%
\begin{figure}
	\includegraphics[width=.95\textwidth]{FIGbkmlattice.pdf}
...
\begin{figure}
	...
	\caption{\label{KMBHtpd} \textbf{(a)} 利用式\eqref{fukui}和文献\cite{Fukui2007}给出的数值方法计算的$\mathbb Z_2$不变量，对于最上面的两个能带，在$nU/t=1$和$\lambda_s/t=0.06$时。由蓝点线标出的边界是通过一个完全数值的计算得到的（具体见式\eqref{lv}上面的解释）。红实线是通过一个级数展开得到的，见式\eqref{lv}。
...
\begin{figure}
	\includegraphics[width=.95\textwidth]{FIGbbhzlattice.pdf}
	\caption{\label{bbhzlattice}
	在正方晶格上的BHZBH模型。
...
\begin{figure*}
	...
	\caption{\label{bbhzbe} Bogoliubov excitation spectrum (blue dots) under full periodic boundary conditions along high symmetric lines in the 1BZ [shown in the inset of left figure], for (left) $m_z/t=2.10$, (middle) $m_z/t=m_z^\star/t\approx 4.48$ and (right) $m_z/t=5.10$. The noninteracting bands (shifted upwards by $nU/2-\mu$) are also plotted in gray lines. Note when the gap of Bogoliubov excitation spectrum just closes, the gap corresponding to the noninteracting band has already reopened as shown in inset of the middle figure. Other relevant parameters: $nU/t=t_s/t=1$.}
\end{figure*}
...
\begin{figure}
	\includegraphics[width=.45\textwidth]{FIGbbhztpd.pdf}
...
最后，我们必须指出，虽然对于弱相互作用的玻色子，Bogoliubov理论的有效性总所周知。利用数值方法，或者多体微扰论去探索更高阶的近似，考虑量子修正，并且研究赝时间反演对称性和其相关的拓扑是否仍然存在，也是一个有趣的课题。
